...
class Robot:
    ...
    def __init__(self, id_client):
        ...
        self.tcp_list = {}  # 保存命名的 TCP 变换矩阵，key 是字符串，value 是 4x4 np.array

    def add_tcp(self, name, tcp_transform_matrix):
        """
        添加一个 TCP 坐标变换矩阵（相对于末端坐标系）
        :param name: TCP 名称（如 'default_gripper'）
        :param tcp_transform_matrix: 4x4 齐次变换矩阵
        """
        assert tcp_transform_matrix.shape == (4, 4), "TCP 变换矩阵必须是 4x4"
        self.tcp_list[name] = tcp_transform_matrix

    def transform_tcp_target_to_ee_target(self, tcp_target_matrix, tcp_name,inverse=False):
        """
        将目标 TCP 坐标系在Base下的位置转换为末端坐标系在Base下的位置。
        :param tcp_target_matrix: TCP 的目标位置（在 base 坐标系下的 4x4 矩阵）
        :param tcp_name: TCP 名称
        :return: 应该传入 ikpy 的末端执行器目标矩阵
        """
        if tcp_name not in self.tcp_list:
            raise ValueError(f"TCP '{tcp_name}' 未定义")


        tcp_in_ee = self.tcp_list[tcp_name]
        if inverse:
            # 如果需要反转变换
            # 将末端坐标系下的目标转换为 TCP 坐标系下的目标:T_tcpinW = T_einW @ T_einTcp
            ee_target_matrix = tcp_target_matrix
            tcp_target_matrix = ee_target_matrix @ tcp_in_ee
            return tcp_target_matrix
        else:
            # T_einW = T_tcpinW @ T_einTcp
            ee_target_matrix = tcp_target_matrix @ np.linalg.inv(tcp_in_ee)
            return ee_target_matrix
    ...
    def show_link_sys(self, linkIndex=0, lifetime=0, type=0,name=None):
        ...
        if name is not None:
            ...
            if not self.debug_lines.get(name):
                self.debug_lines[name] = {}

            self.debug_lines[name]['line_x_axis'] = p.addUserDebugLine(endEffectorPos, endEffectorPos + x_axis, lineColorRGB=[1, 0, 0], lineWidth=2,
                               lifeTime=lifetime if lifetime >= 0 else 0,
                               replaceItemUniqueId=self.debug_lines[name]['line_x_axis'] if (self.debug_lines and
            ...
            self.debug_lines[name]['line_y_axis'] = p.addUserDebugLine(endEffectorPos, endEffectorPos + y_axis,
                                                                       lineColorRGB=[0, 1, 0], lineWidth=2,
                                                                       lifeTime=lifetime if lifetime >= 0 else 0,
                                                                       replaceItemUniqueId=self.debug_lines[name][
                                                                           'line_y_axis'] if (self.debug_lines and
                                                                                   self.debug_lines.get(name) and
            ...
            self.debug_lines[name]['line_z_axis'] = p.addUserDebugLine(endEffectorPos, endEffectorPos + z_axis,
                                                                       lineColorRGB=[0, 0, 1], lineWidth=2,
                                                                       lifeTime=lifetime if lifetime >= 0 else 0,
                                                                       replaceItemUniqueId=self.debug_lines[name][
                                                                           'line_z_axis'] if (self.debug_lines and
                                                                                   self.debug_lines.get(name) and
                                                                                   self.debug_lines.get(name).get(
                                                                                       'line_z_axis') and
                                                                                   lifetime == -1) else -1,
            ...
            # 绘制Z轴（蓝色）
            self.line_z_axis = p.addUserDebugLine(endEffectorPos, endEffectorPos + z_axis, lineColorRGB=[0, 0, 1],
                                                  lineWidth=2,
                                                  lifeTime=lifetime if lifetime >= 0 else 0,
    ...
    def load_urdf(self, *args, **kwargs):
        ...
        # self.collision_self_exclude_parent()  # 过滤碰撞对
    ...
    def get_pos_ori_from_ik(self, joints=None,tcp_name=None):
        """
        通过正运动学计算末端执行器的位置和姿态
        :param joints: 关节角度列表
        :return: 末端执行器的位置和姿态
        ...
        joints = self.apply_mask_to_joint_vector(joints, self.robot_chain.active_links_mask)
        # 计算末端执行器的位姿
        end_effector_frame = self.robot_chain.forward_kinematics(joints)
        if tcp_name is not None:
            # 如果指定了 TCP 名称，则将末端执行器坐标系转换为 TCP 坐标系
            end_effector_frame = self.transform_tcp_target_to_ee_target(end_effector_frame, tcp_name,inverse=True)
        pos, ori = self.matrix_to_pos(end_effector_frame)
        return pos, ori

    def get_state_from_ik(self, pos, ori, damping=(0.1,), start=None, currentPosition=None,maxNumIteration=100,tcp_name=None):
        if tcp_name is not None:
            # 如果指定了 TCP 名称，则将目标位置和姿态转换为末端执行器坐标系下的目标
            if ori is None:
                raise ValueError("如果指定了 TCP 名称，则必须提供姿态")
            tcp_in_W_matrix = self.pos_to_matrix(pos, ori)
            ee_in_W_matrix = self.transform_tcp_target_to_ee_target(tcp_in_W_matrix, tcp_name)
            pos, ori = self.matrix_to_pos(ee_in_W_matrix)
        ...
        if self.inverse_mode=='world_sys':
            ...
            # 构造initial_position
            if start is not None:
                initial_position = self.apply_mask_to_joint_vector(start, self.robot_chain.active_links_mask)
            else:
                start = current_joint_positions
                initial_position = self.apply_mask_to_joint_vector(start, self.robot_chain.active_links_mask)

            # 逆运动学求解（完整链）
            full_joint_result = self.robot_chain.inverse_kinematics_frame(
                target=transform_matrix,
                initial_position=initial_position,
                orientation_mode='all' if ori is not None else None
            )

            # 提取出激活关节对应的值（去掉第一个固定基座的关节）
            joints_value = self.extract_masked_joints(full_joint_result, self.robot_chain.active_links_mask)



        return joints_value

    def apply_mask_to_joint_vector(self,joint_values, mask, fill=0.0):
        """
        将短的joint_values按mask插入到对应True的位置，其他位置填充fill
        """
        full = []
        idx = 0
        for active in mask:
            if active:
                full.append(joint_values[idx])
                idx += 1
            else:
                full.append(fill)
        return full

    def extract_masked_joints(self,full_joint_values, mask):
        """
        从完整的joint_values中提取出mask为True的关节值
        """
        return [j for j, m in zip(full_joint_values, mask) if m]
    ...
    def calculate_ee_origin_from_target(self,target_point_world,target_ori_world, point_in_ee_frame, ori_in_ee_frame=[0,0,0,1],inverse=False,ee_origin_world=None,ee_orientation_world=None):
        """
    ...
    def joint_move(self,joint_positions,force=10000,maxVelocity=1):
        """ 控制机械臂的关节 """
