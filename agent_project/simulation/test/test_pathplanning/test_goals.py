...
from agent_project.simulation.test.robot_ghost import *
...
def test_goals():
    ...
    machine_state = scene_config["machine_state_B"]
    ...
    goal_states, stats = expand_goal_states_nearby(
        goal_q=goal,
        fk_fn=fk_fn,
        ik_fn=ik_fn,
    ...
    print(f"Generated {len(goal_states)} valid goal states around the original goal.")

    for i, q in enumerate(goal_states):
        sim_env.robot_list[0].set_joints_states(q)
        time.sleep(0.1)
    ...
    res, ori_path, path, solved_time, total_states = sim_env.pb_ompl_interface.plan_start_to_goal_states(start,goal_states,allowed_time=30,ret_all=True)
    ...
    stage_colors = [
        (0.3, (0.2, 0.4, 1.0, 0.08)),  # 前段：蓝色
        (0.7, (1.0, 0.8, 0.1, 0.18)),  # 中段：黄色
        (1.0, (1.0, 0.2, 0.2, 0.35)),  # 后段：红色
    ]

    # ghost_ids, used_indices, ee_marker_ids = create_robot_ghost_trajectory(
    #     robot=sim_env.robot_list[0],
    #     trajectory=path,
    #     max_snapshots=5,
    #     sample_mode="progress",
    #     snapshot_progresses=[0, 0.4, 0.7, 0.85, 0.9],
    #     rgba_start=(0.3, 0.3, 0.3, 0.05),
    #     rgba_end=(0.9, 0.9, 0.9, 0.5),
    #     # stage_colors=stage_colors,
    #     draw_ee_points=False,
    #     ee_link_index=sim_env.robot_list[0].id_end_effector,
    # )

    # ee_marker_ids, point_indices = create_ee_path_points(
    #     robot=sim_env.robot_list[0],
    #     trajectory=path,
    #     ee_link_index=sim_env.robot_list[0].id_end_effector,
    #     point_stride=5,  # 每5个轨迹状态画一个点
    #     point_radius=0.003,
