...
class SimulationEnvironment:
    ...
    def load_scene(self):
        ...
        self.workpiece_pose = [0., 0.06, 0.082]
        ...
        self.rm_sys.create_constrain(machine,self.robot_list[0],parentPosition=[-0.18, +0.1 , 0.12], childOrientation=[0.7068252, 0, 0, 0.7073883]
...
def export_path():
    ...
    file_path = r'/home/lwh/Project/python_project/Ai_agent/agent_project/simulation/test/test_pathplanning/path_files/圆弧工件_固定刀轴矢量.cls'
    ...
    target_points_in_robot_sys = [(pos_list[i], ori_list[i]) for i in selected_idx]
    target_point_in_robot_sys = target_points_in_robot_sys[0]
    ...
    path = []
    pairs = zip(pos_list[609:], ori_list[609:])
    total_steps = min(len(pos_list[609:]), len(ori_list[609:]))
    ...
    if collision_detect:
        ...
        for i, (pos, ori) in enumerate(tqdm(pairs, total=total_steps, desc="Planning path")):
            ...
            path.append(joints)
            # time.sleep(0.1)
    np.save("./path_files/圆弧工件_新装置.npy", np.array(path))
    print(f"Saved executed path with {len(path)} points to ./path_files/圆弧工件_新装置.npy")
    time.sleep(2)
...
def debug_path():
    ...
    joints = np.load("/home/lwh/Project/python_project/Ai_agent/agent_project/simulation/test/test_pathplanning/path_files/圆弧工件_新装置.npy")

    goal = joints[0]
    # 设置机械臂/机床的初始位置
    start = [0.10568717528231546, -0.4105353654619583, -1.1813494586720104, 0.03241272300551933, -1.8410220234890533,
             -0.6757545624540242]
    start_point_in_robot_sys = sim_env.rm_sys.get_point_in_workpiece2robot([0.0, -0.090, 0.040],
...
def export_path2():
    ...
    """
    await robot.joint_move(joints[0],speed=0.1)
    n=5
    speed=50
    for i in range(n):
        robot.robot.linear_move_extend([0,70,0,0,0,0], 1, True, speed,200,0.001)
        robot.robot.linear_move_extend([0,0,2,0,0,0], 1, True, speed,200,0.001)
        robot.robot.linear_move_extend([0,-70,0,0,0,0], 1, True, speed,200,0.001)
        robot.robot.linear_move_extend([0,0,2,0,0,0], 1, True, speed,200,0.001)

    robot.robot.linear_move_extend([0,0,-2*2*n,0,0,0], 1, True, speed,200,0.001)
    :return:
    """
