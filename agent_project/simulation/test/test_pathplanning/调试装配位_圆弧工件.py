...
class SimulationEnvironment:
    ...
    def load_scene(self):
        ...
        robot_with_rolling_tool_urdf = r"./models/jaka_description/urdf/jaka_minicobo_with_rolling_tool_12mm_len120mm.urdf"
...
def export_path():
    ...
    if collision_detect:
        ...
        for i, (pos, ori) in enumerate(tqdm(pairs, total=total_steps, desc="Planning path")):
            ...
            path.append(joints)
            # time.sleep(0.1)
    np.save("./path_files/圆弧工件.npy", np.array(path))
    print(f"Saved executed path with {len(path)} points to ./path_files/圆弧工件.npy")
    time.sleep(2)
...
def debug_path():
    ...
    joints = np.load("/home/lwh/Project/python_project/Ai_agent/agent_project/simulation/test/test_pathplanning/path_files/圆弧工件.npy")
