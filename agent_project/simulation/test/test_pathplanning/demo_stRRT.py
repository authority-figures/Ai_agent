...
scene_config={
    "machine_state_A": np.array([0.0, -0.0, 0.4, 0.1, 0.0]),
    "machine_state_B": np.array([0.0, -0.0, 0.4, 0.4, 0.0]),
...
def planning(
    ...
    sim_env.pb_ompl_interface.z_range = (0.01, 0.2)
...
def save_planner_results_npz(out_dir, scene_name, planner_name, results, summary, *, extra_meta=None):
    ...
    solved_time = np.array([r["solved_time"] for r in results], dtype=float)
    total_states = np.array([r["total_states"] for r in results], dtype=int)

    start = np.array([r["start"] for r in results], dtype=float)
    noise = np.array([r["noise"] for r in results], dtype=float)

    # ---- 变长对象：路径（必须用 object）----
    ori_path = np.array([r["ori_path"] for r in results], dtype=object)
    path = np.array([r["path"] for r in results], dtype=object)

    # ---- summary / meta（dict 直接 object 存）----
    summary_obj = np.array(summary, dtype=object)
    meta_obj = np.array(extra_meta, dtype=object)
...
def planning_all_planners():
    out_dir = r"/home/lwh/Project/python_project/Ai_agent/agent_project/simulation/test/test_pathplanning/datas/20260114"
    ...
    for scene_name in list(scene_config.keys())[3:]:
        ...
        for planner_name in planners:
            ...
            results, summary = planning(sim_env, start, goal, planner_name, repeat=10)
            ...
            save_planner_results_npz(out_dir, scene_name, planner_name, results, summary, extra_meta=meta)
            meta={}
